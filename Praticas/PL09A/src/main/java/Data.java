...
public class Data implements Comparable<Data> {
    ...
    public final void setData(int ano, int mes, int dia) {
        this.ano = ano;
            if (mes < 1 || mes > 12) {
                throw new MesInvalidoException(String.format("Mes %d invalido",mes));
            }
            if (dia <= 0 || dia > Mes.obterMes(mes).numeroDeDias) {
                throw new DiaInvalidoException(String.format("Dia %d invalido no mes %s", dia, Mes.obterMes(mes).toString()));
            }
            this.mes = Mes.obterMes(mes);
            this.dia = dia;
    }
